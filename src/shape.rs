...
impl Shape {
    ...
    pub fn to_geo_types(v: &[Shape]) -> LineString {
        let coords: Vec<Coord> = v.iter().map(|node| {
            Coord {
                x: node.longitude,
                y: node.latitude
            }
    ...
    pub fn to_geo_types_interp(v: &[Shape], start: f32, end: f32) -> LineString {
        ...
        let coords: Vec<Coord> = v[start_idx..=end_idx].iter().map(|node| {
            Coord {
                x: node.longitude,
                y: node.latitude
            }
        ...
        if end_frac > 0.0 || !end_is_whole {
            let end_coord = Shape::interpolate(&coords[length - 2], &coords[length - 1], end_frac as f64);
...
impl FromWithAgencyId<gtfs_structures::Shape> for Shape {
    fn from_with_agency_id(agency_id: u8, f: gtfs_structures::Shape) -> Self where Self: Sized {
        Shape {
